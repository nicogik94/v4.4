# Strategic Decision Audit Demo Script

This is a talk track for the Strategic Decision Audit packaged offer. It uses
the existing local operator workflow. It is not legal advice, not financial
advice, not public SaaS, not a guaranteed recommendation engine, and not a new
reasoning mode.
...
Strategic Decision Audit packages the current Decision Engine around one clear
operator workflow: intake a strategic decision, run the existing eight-phase
engine, review missing information, and export client-safe and operator-only
artifacts.
...
The engine does not make the decision. It produces structured review material:
...
5. Paste the example brief from
   [`examples/strategic-decision-audit-brief.md`](examples/strategic-decision-audit-brief.md).
...
10. Open **Overview**, **Dossier**, **Report**, and **Control log** as the run
    completes.
11. Review operator review support for missing information.
...
- "These hypotheses guide what evidence to test next. They are not conclusions
  by themselves."
- "Client-safe means cleaned for review, not guaranteed correct or legally
  approved."
...
- confirm the recommendation matches the decision question,
...
- route legal, financial, compliance, safety, or domain claims to the right
  reviewer,
...
- No auth, tenancy, public deployment hardening, provider routing changes,
  queue/runtime changes, report rewrite, or export schema changes.
